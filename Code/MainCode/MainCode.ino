...
void loop() {
  ...
  if(setupVar == true){
    ...
    if(millis() - lastCheckPressureMillis >= checkPressureInterval){ 
      ...
      Serial.println(" mbar");
      */
      send_serial_data(BackPumpStatus, MotorPumpStatus, PressureValue, param["Pressure1"].receivedValue, param["TempElec"].receivedValue, param["TempMotor"].receivedValue, param["TempPmpBot"].receivedValue, screen_state, arrow_state);
...
void send_serial_data(bool BStatus, bool PStatus, double pressure1, char *pressure2, char *Temp1, char *Temp2, char *Temp3, int screen_state, int arrow_state){
  //String str1=S
  String array=String(BStatus)+"h"+String(PStatus)+"h"+String(pressure1)+"h"+String(pressure2)+"h"+String(Temp1)+"h"+String(Temp2)+"h"+String(Temp3)+"h"+screen_state+"h"+arrow_state;
